...
//enqueue - Add qeueue
void addNode (CreateMovieNode*& head, CreateMovieNode*& tail, CreateMovieNode* nodeptr);

//eqeueue - dElete queue (takes in actual struct and modifies them.)
CreateMovieNode *deleteNode (CreateMovieNode*& head, CreateMovieNode*& tail);
    ...
    if (fin.good()) // only read from the file if it exists!
        ...
        while (true || bufS != "EOF")
            ...
            */
            ...
            addNode(head, tail, storageInput);
...

{
    ...
    addNode(head, tail, inputNewMovieNode);
...
void funcChoiceRemove(CreateMovieNode*& head)
{
	...
	if(cVal != true)
    {
        ...
        int index = 0; //for counting the list.
        ...
        int sequenceNumber; //input to pick which node to delete
        ...
        if (sequenceNumber == 1)
            {
            //sets the address of head (which is a node) and links temp1 to it.
