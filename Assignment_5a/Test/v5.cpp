...
void toLowerCase (string& input); //c++ string to lowercase

Calculate toLowerCaseCstring (struct Calculate s,int n); //c string to lowercase.
...
void function3(struct Calculate s)
{
    ...
    toLowerCase(input); //Lower Cases input
    ...
    //storage for C lowercase function since its pass by value -has a copy.
    Calculate store2;
    ...
    for (int n = 0; n < s.counter; n++ )
    {
        ...
        store2 = toLowerCaseCstring(s, n); //runs C char to lower case function and store values
        strcpy(s.arrayDesc[n],store2.arrayDesc[n]); //copies new data inside.
...
void toLowerCase (string& input)
{
    ...
    //pass by references so changes in value -no need for return.
}

Calculate toLowerCaseCstring (struct Calculate s,int n)
{
    //int x = string length of s.arrayDesc[n]
